...
This problem can be easily dealt with if we only require $\kernel{K}$ to be unique up to a set of measure 0 with respect to some measure $\prob{P}^*\in\Delta(\sigalg{D}\otimes\sigalg{E})$, because we can simply take $\kernel{K}$ to be an arbitrary disintegration of $\prob{P}^*$ and then use Theorem \ref{th:iterated_disint} to find further disintegrations. However, sometimes it appears that this requirement is \emph{not} satisfied. An example involving Causal Bayesian Networks is given below.

Suppose we have $\RV{X}$ and $\RV{Y}$ in $[0,1]$, the graph $\mathcal{G}:=\RV{X}\rightarrow \RV{Y}$ and, as above, the observational distribution is $\prob{P}(\RV{X}\in A,\RV{Y}\in B)=\lambda(A)\lambda(B)$. Then, because elements of $\prob{P}^{\{\RV{Y}|\RV{X}\}}$ is unique up to a set of $\kernel{P}$-measure 0, Equation \ref{eq:non_measurable_disint} is a version of $\prob{P}^{\{\RV{Y}|\RV{X}\}}$ for each $d\in D$. Identify each $d\in D$ with an intervention $do(\RV{X}=x)$. According to the definition of \citet{pearl_causality:_2009} pg 24, the interventional distribution $\prob{P}(d)(\RV{X},\RV{Y})$ must have the following properties for evey $x\in X$:
...
\begin{itemize}
	...
	\item Let $\prob{T}^{\RV{XY}|\RV{X}}$ be a particular version of $\prob{P}^{\{\RV{XY}|\RV{X}\}}$. Then $\prob{P}_{do(\RV{X}=x)}^{\RV{XY}|\RV{X}}\in \prob{T}^{\{\RV{XY}|\RV{X}\}}$, $\prob{P}_{do(\RV{X}=x)}$-almost surely
\end{itemize}
...
Or, indeed, any function at all $X\times X\to \Delta(\sigalg{X}\otimes\sigalg{Y})$. That is, for every $x\in X$, $\RV{Y}$ is almost surely an indicator of membership of $x$ in the non-measurable set $C$. This is a nonsensical result which has very little to do with the original $\prob{P}$ but it is also completely impossible to determine the result of any mixture of $do$-operations.

Alternatively, we may take the second interpretation, in which case we can avoid nonmeasurability. However, letting $R$ be the Cantor set which has an uncountable number of elements and $\lambda(R)=0$, it is still consistent to define
...
While Equation \ref{eq:cbn_cont2} behaves ``as it is supposed to'' for mixtures of $do$ operations $\pi\in \Delta([0,1])$ for which $\lambda\gg \pi$, it also sets $\RV{Y}$ to 1 for any point interventions in the Cantor set, or any mixtures of $do$ operations absolutely continuous with respect to the Cantor set.
