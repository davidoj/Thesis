...
For the following lemma, we have to deal with two issues: first, we require an infinite sequence of consequences in order to have an exact representation theorem like Theorem \ref{th:rep_seedo_obs}. At the same time, we want a denumerable set of choices $\RV{D}$ in order to avoid concerns regarding the existence of disintegrations. For this, we define a \emph{default extension} of an functionally exchangeably extendable see-do forecast that extends the consequences without adding extra choices.

\begin{definition}[Default extension]
Given an infinitely functionally exchangeably extendable see-do forecast $(\kernel{T},\RV{D},\RV{X},\RV{Y})$ with respect to $\RV{D}=\utimes_{i\in A} \RV{D}_i$ and $\RV{Y}=\utimes_{i\in A} \RV{Y}_i$ ($A\subset \mathbb{N}$ finite), take some infinitely exchangeable $\prob{P}\in\Delta(\sigalg{D}_1^{\mathbb{N}})$ and define  such that for any $\mathbf{d}\in D_1^A$, $\prob{P}^{\RV{D}_{A}}(\{d\})>0$
\end{definition}

\todo[inline]{Mention that an approximate version is probably neater}
