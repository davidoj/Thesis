...
Given measureable space $(E,\sigalg{E})$, a sequence of random variables $\RV{X}:=(\RV{X}_i)_{i\in A}$, a random variable $\RV{Y}$ with $\sigma(\RV{X})\cap \sigma(\RV{Y})=\{\emptyset,E\}$ and a finite permutation $\rho:A\to A$, the swap function $\rho^{\RV{X}}:E\to E$ that holds $\RV{Y}$ fixed is a $\sigalg{E}$-measurable function for which $\RV{X}_i\circ \rho^{\RV{X}}=\RV{X}_{\rho'(i)} $ for all $i\in A$, and $Y\circ\rho^{\RV{X}}=\RV{Y}$.
\end{definition}

For example, and as a proof of existence, if $E = Y\times X_0^{|A|}$, $\RV{X}_i:E\to X_0$ projects the $i$-th ``x'' element of the sequence $(y,x_1,...,x_i,...)\mapsto x_i$, $\RV{X}=(\RV{X}_i)_{i\in A}$ then for any finite permuation $\rho:A\to A$ the swap function is the the fuction $\rho^{\RV{X}}:(y,x_1,...,x_i,...)\mapsto (y,x_{\rho(1)},...,x_{\rho(i)},...)$.

If we are only interested in the sequence $\RV{X}$ then we can choose $\RV{Y}=\mathds{1}_E$, for which the second condition will always be satisfied.

Note that a swap function $\rho^{\RV{X}}$ also has an associated Markov kernel $\kernel{F}_{\rho^{\RV{X}}}$. The product of the swap kernel with the kernel associated with the original random variable $\RV{X}$ is the permuted version of $\RV{X}$:

\begin{align}
    \kernel{F}_{\rho^{\RV{X}}} \kernel{F}_{(\RV{X},\RV{Y})} &= \kernel{F}_{\rho^{\RV{X}}} (\utimes_{i\in B} \kernel{F}_{\RV{X}_i})\utimes \kernel{F}_{\RV{Y}} \\
    &= (\utimes_{i\in B} \kernel{F}_{\rho^{\RV{X}}} \kernel{F}_{\RV{X}_{i}})\utimes \kernel{F}_{\rho^{\RV{X}}} \kernel{F}_{\RV{Y}} \label{eq:determ_commute}\\
...
We will use this property to define exchangeability. As with swap functions, we will define exchangeability with respect to a sequence and an additional random variable that should remain untouched.
...
Given a probability space $(\kernel{P},(O,\sigalg{O}))$ exchangeable with respect to $\RV{X}:=(\RV{X}_i)_{i\in A}$ (where $\RV{X}_i:O\to X_0$ for all $i\in A$) and $\RV{Y}:O\to Y$, where $O$ is standard measurable and $A$ is countably infinite, then there exists some $\RV{Z}:O\to Z$ such that $\sigma(\RV{Z})\subset \sigma(\RV{X})$ and both of the following hold:
\begin{enumerate}
...
\begin{proof}
We have immediately that $(\mu\otimes \mathrm{Id}_D)\kernel{K}$ is a Markov kernel $D\to \Delta(\sigalg{X}\otimes\sigalg{Y})$. We require $\RV{X}\CI_{\kernel{L}}\RV{D}$. By Theorem \ref{th:iterated_disint} we have
...
Given a see-do forecast $(\kernel{T},\RV{D},\RV{X},\RV{Y})$ functionally exchangeable with respect to $\RV{Y}=(\RV{Y}_i)_{i\in A}$ and $\RV{D}=(\RV{D}_i)_{i\in A}$ holding $\RV{X}$ fixed, some $\prob{P}^{\RV{D}}\in \Delta(\sigalg{D})$ exchangeable with respect to $\RV{D}$, then $\prob{P}^{\RV{D}}\kernel{T}\in \Delta(\sigalg{D}\otimes\sigalg{X}\otimes\sigalg{Y})$ is exchangeable with respect to $(\RV{D}_i,\RV{Y{_i}})_{i\in A}$ holding $\RV{X}$ fixed.
...
We will first prove a slight extension of Theorem \ref{th:de_finetti_rep_thm}: we will show that a see-do forecast with exchangeable observations induces a conditionally IID statistical model over observations. The main addition here is that we need to show that exchangeability ``holding the consequences $\RV{Y}$ fixed'' additionally implies conditional independence of $\RV{X}$ and $\RV{Y}$ given $(\RV{Z},\RV{D})$.
...
\subsubsection{Proof of Theorem \label{sec:de_finetti_rep_thm}}
