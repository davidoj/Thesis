...
Here we review the key representation theorems of \citet{savage_foundations_1954} and \citet{jeffrey_logic_1990}, along with the theory of \citet{lewis_causation_1986} that was presented as an alternative to Jeffrey's. All theories have a  We establish that both imply that choices are compared using a probabilistic function $C\to \Delta(\Omega)$ for a suitable selection of $C$ and $(\Omega,\sigalg{F})$, along with a ``desirability'' function which differs in type between the two theorems.
