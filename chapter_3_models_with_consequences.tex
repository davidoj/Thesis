...
Given that counterfactual questions are suggested to be the most general kind of causal question, one might ask why this work focuses on questions of an interventional nature. There are two reasons for this. Firstly, the theory presented here doesn't start with CBNs or SCMs, so the motivation is relying only on the informal elements of the ladder rungs. I have much stronger intuitions about decision problems than counterfactual queries, and questions about the formal representation of decision problems are also much more studied than questions about the formal representation of counterfactual queries. This all means it's much easier to propose a formalisation of decision making than it is to propose a formalisation of counterfactual speculation.

Secondly, Rung 2 questions are far more prominent in the causal inference literature than rung 3. \citet{rubin_causal_2005} argues that causal inference often to informs a decision maker by providing ``scientific knowledge'', but does not make recommendations by itself. \citep{imbens_causal_2015} introduces causal inference as the study of ``outcomes of manipulations'' and \citep{spirtes_causation_1993} highlights the universal relevance of understanding how to control certain outcomes, while further arguing that clarifying commonsense ideas of causation is also an important aim of causal inference. \citet{hernan_whatif_2020} present causal knowledge as critical for assessing the consequences of actions. Problems that involve comparing different choices on the basis of their consequences are an important class of problems. They are worth better understanding, even if this work does not shed any light on counterfactual questions.

At the informal level, I also wonder whether counterfactual queries could all be formulated as decision problems with fanciful options. An example of an informal decision problem is ``I want my headache to go away; should I take Aspirin?'' while an example of a counterfactual query is ``if I had taken the Aspirin, would I still have a headache?''. However,  we could also pose a fanciful decision problem ``I want my headache to go away; should I perform a magic ritual to make it so that I have already taken Aspirin?''. This is not a nonsensical decision problem, as the act in question cannot actually be taken. Nevertheless, whatever means we have of evaluating the state of affairs under alternative histories can also be used to evaluate the consequences of magic rituals that alter history. So, speculatively, a formalisation of consequences of decisions may also serve as a formalisation of ``counterfactual consequences'' if it admits a sufficiently wide variety of options.
...
A statistical decision problem is a tuple $(X, H, D, l, \prob{P}_\cdot)$ where $(X,\sigalg{X})$ is a set of outcomes, $(H,\sigalg{H})$ is a set of hypotheses, $(D,\sigalg{D})$ is a set of decisions, $l:D\times H\to \mathbb{R}$ is a loss function and $\prob{P}_\cdot:H\kto X$ is a Markov kernel from hypotheses to to outcomes.
\end{definition}
...
SDT offers two decision rules that can be used to determine a best decision function when the hypothesis is unknown. The first is to choose the decision function minimising the \emph{Bayes risk}, which requires a prior over hypotheses. The second decision rule is to choose the decision rule minimising the maximum risk, and this rule does not require a prior.

\begin{definition}[Bayes risk]
Given a statistical decision problem $(X, H, D, l, \prob{P}_\cdot)$, decision functions $C$ and a prior $\mu\in \Delta(\sigalg{H})$, the \emph{Bayes risk} is
\begin{align}
    R_\mu(\kernel{D}_\alpha) &:= \int_{H} R(\kernel{D}_\alpha,h)\mu(\mathrm{d}h)\\
    &=\int_X \int_D l(d,h) \kernel{D}_\alpha(\mathrm{d}d|f)\kernel{P}_h(\mathrm{d}f)\mu(\mathrm{d}h)
\end{align}
\end{definition}

\begin{definition}[Minimax rule]
Given a statistical decision problem $(X, H, D, l, \prob{P}_\cdot)$ and decision functions $C$, a \emph{minimax decision function} is any decision function $\kernel{D}_\alpha$ satisfying
\begin{align}
...
To create a model \emph{with consequences} that induces the SDT risk, we need to make a few assumptions. Recall that the basic model for a decision problem is either a Markov kernel $\prob{P}_{\cdot}:C\kto \Omega$ or $\prob{P}_{\cdot,\cdot}:C\times H\kto \Omega$ for some set of hypotheses $H$, and that a utility function $u$ is also available. Here, $\Omega$ represents both observations (if any are taken) and consequences of decisions. Statistical decision problems correspond to a particular instance of this general scheme. In particular, statistical decision problems are associated with probability functions in which:
\begin{itemize}
    \item The set of ``choices'' is identified with the set of decision rules
    \item There is some $\RV{X}:\Omega\to X$ representing the observations made before any decision rule is enacted
    \begin{itemize}
        \item Given any hypothesis, observations are independent of the choice of decision rule
    \end{itemize}
    \item There are consequences $\RV{Y}:\Omega\to Y$ and decisions $\RV{D}:\Omega\to D$
    \begin{itemize}
        \item Given a hypothesis and a decision, the consequences $\RV{Y}$ are independent of both the choice of decision rule and the observations
        \item Given a decision function, the decision $\RV{D}$ is independent of the hypothesis
    \end{itemize}
    \item The utility $u:Y\to \mathbb{R}$ is a function of the consequences
\end{itemize}

We call models satisfying these requirements \emph{see-do models}, because they feature observations (the ``see'' part) along with decisions and consequences (the ``do'' part), and the observations come ``before'' the decisions (hence see-do).
...
The probability set model induces a set of decision functions: for each $\alpha\in C$, there is a probability distribution $\prob{P}_\alpha^{\RV{D}|\RV{X}}$.

\begin{theorem}[Induced risk]
Given a see-do model $(\prob{P}_{C\times H}, \RV{X},\RV{Y},\RV{D})$ along with a utility $u:Y\to \mathbb{R}$, the expected utility for each choice of decision rule $\alpha\in C$ and hypothesis $h\in H$ pair is equal to the negative risk of the decision rule and hypothesis pair.
\end{theorem}
...
We can also define a ``Bayesian probability set model'', which is a probability set model $\prob{P}_C$ along with a prior $\prob{P}_C^{\RV{H}}$ over hypotheses. In this case, the expected utility of a decision rule is equal to the Bayes risk.
...
\begin{align}
    ...
    \RV{Y}&\CI^e_{\prob{P}_{C}} (\RV{X,C})|(\RV{D},\RV{H})\\
    \RV{D}&\CI^e_{\prob{P}_{C}} \RV{H}|\RV{C}\\
    \RV{H}&\CI^e_{\prob{P}_C} \RV{C}
\end{align}
\end{definition}

\begin{theorem}[Induced Bayes risk]
Given a Bayesian see-do model $(\prob{P}_{C}, \RV{X},\RV{Y},\RV{D},\RV{H})$ of a statistical decision problem along with a utility $u:Y\to \mathbb{R}$, the expected utility for each choice of decision rule $\alpha\in C$ is equal to the negative Bayes risk of that decision rule relative to the prior $\prob{P}_C^{\RV{H}}$.
\end{theorem}

\begin{proof}
The expected utility of $\alpha\in C$ is 
\begin{align}
    \int_Y u(y) \prob{P}_{\alpha}^{\RV{Y}}(\mathrm{d}y) &= \int_Y  \int_D \int_{X} \int_H u(y)\prob{P}_{\alpha}^{\RV{Y}|\RV{DXH}}(\mathrm{d}y|d,x,h)\prob{P}_{\alpha}^{\RV{D}|\RV{XH}}(\mathrm{d}d|x,h)\prob{P}_{\alpha}^{\RV{X}|\RV{H}}(\mathrm{d}x|h)\prob{P}_\alpha^{\RV{H}}(\mathrm{d}h) \\
    ...
    &=  \int_X  \int_D \int_{Y} \int_H u(y) \prob{P}_{C}^{\RV{Y}|\RV{DH}}(\mathrm{d}y|d,h)\prob{P}_{\alpha}^{\RV{D}|\RV{X}}(\mathrm{d}d|x)\prob{P}_{C}^{\RV{X}|\RV{H}}(\mathrm{d}x|h)\prob{P}_C^{\RV{H}}(\mathrm{d}h)\\
     &= -\int_D\int_X\int_H l(d,h)\prob{P}_{\alpha}^{\RV{D}|\RV{X}}(\mathrm{d}d|x)\prob{P}_{C}^{\RV{X}|\RV{H}}(\mathrm{d}x|h)\prob{P}_C^{\RV{H}}(\mathrm{d}h)\\
    &= -\int_H R(\prob{P}_{\alpha}^{\RV{D}|\RV{X}},h)\prob{P}_C^{\RV{H}}(\mathrm{d}h)
    &= -R_{\prob{P}_C^{\RV{H}}}(\prob{P}_{\alpha}^{\RV{D}|\RV{X}})
\end{align}
\end{proof}

\begin{theorem}[Extension to a Bayesian see-do model]
Given a see-do model $(\prob{P}_{C\times H}, \RV{X},\RV{Y},\RV{D})$ 
\end{theorem}

\subsubsection{Complete class theorem}\label{sec:cc_theorem}

A Bayesian see-do model is a probability set $\prob{P}_C$ and a see-do model is a probability set $\prob{P}_{C\times H}$. We can obtain the former by combining the latter with a prior $\mu\in \Delta(H)$; call this an induced Bayesian model with respect to $\mu$. The \emph{complete class theorem} establishes that any \emph{admissible} decision rule for a see-do model $\prob{P}_{C\times H}$ must minimise the Bayes risk for the Bayesian model induced by some $\mu\in \Delta(H)$. This theorem is somewhat similar in content to the other representation theorems discussed in this chapter -- given a see-do model, a utility and any means of selecting an admissible decision function, there is a Bayesian ``representation'' of this selection rule obtained by extending the model with a prior.

\begin{definition}[Admissible decision rule]\label{def:admissible_decision}
