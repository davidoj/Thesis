...
Given $H$ and $C$ both finite and a risk function $R:C\times H\to \mathbb{R}$ and an associated probability set $\prob{P}_C$, if
\begin{align}
    \prob{P}_{\alpha,h} &= a \prob{P}_{\beta,h} + (1-a)\prob{P}_{\beta',h} \\
    \implies R(\alpha,h)&=aR(\beta,h) + (1-a)R(\beta',h)&\forall h\in H\label{ass:convexity_implied}
\end{align}
and 
\begin{align}
    Q:= (\prob{P}_{\{\alpha\}\times H})_{\alpha\in C}
\end{align}
is convex closed, then the risk set $S$ is convex closed.
\end{lemma}

\begin{proof}
Consider $y,y'\in S$, where
\begin{align}
    y &= (R(\alpha,h))_{h\in H}\\
    y' &= (R(\alpha',h))_{h\in H}
\end{align}
Consider some $0\leq a\leq 1$ and define $y_a=ay + (1-a)y'$. By assumption of convexity of $Q$, there is some $\alpha_a\in C$ such that for all $h$
\begin{align}
    \prob{P}_{\alpha_a,h} &= a\prob{P}_{\alpha,h} + (1-a)\prob{P}_{\alpha',h}
\end{align}
but then by the assumption in Eq. \ref{ass:convexity_implied}
\begin{align}
    y_a &= (R(\alpha_a,h)_{h\in H})
\end{align}
Closure of $S$ follows from the fact that the function $\prob{P}_{\{\alpha\}\times H}\mapsto R(\alpha_a,h)_{h\in H}$ is a continuous function and $Q$ is a compact set
\end{proof}
...
The fact that $S$ is convex and closed follows from Lemma \ref{lem:convex_closed}, and the fact that $\prob{P}_C$ is the convex hull of
\begin{align}
     \prob{P}_{C_0} := \{\prob{P}_\alpha|\prob{P}_\alpha^{\RV{D}|\RV{X}}\text{ deterministic}\}
\end{align}
\end{example}

\begin{example}
As before, but suppose we have the see-do model is not conditionally independent. Then the risk set is still convex and closed downwards, and so the set of Bayes optimal choices is still the set of admissible choices.
\end{example}
