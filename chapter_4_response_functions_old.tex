...
\end{align}
where Equation \eqref{eq:sym} follows from symmetry of $\kernel{L}$ and \eqref{eq:ccont} follows from causal contractibility of $\kernel{M}(\cdot|z,\cdot)$
...
From Equation \eqref{eq:rel_bet_y_yd} and causal contractibility of $\prob{P}_C^{\RV{Y}|\RV{D}}(A|e)$, take some $d\in D^{\mathbb{N}}$ and
\begin{align}
