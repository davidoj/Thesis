...
\begin{align}
    (A \succurly B) \iff (\prob{P}^{\RV{O}|\mathds{1}_A}_1u>\prob{P}^{\RV{O}|\mathds{1}_B}_1u) \label{eq:ev_dec_theory}
\end{align}

The original proof uses an $O$ that is not standard measurable, and states Equation \ref{eq:ev_dec_theory} in terms of the probability conditioned on events. I state it this way so that the notation is consistent with other theories under discussion which I hope helps to discuss the comparison.

In order to use such a model in a decision problem, Jeffrey suggests we can pick a subset of $\mathscr{D}:=\{D_i\in \sigalg{O}|i\in A\}$ for some set $A$ that we identify as ``decisions that we can make''. There is a somewhat subtle issue here. Consider the problem of deciding whether or not to order a drink decision problem with the options $D=\{d_1=\text{``order water''},d_2=\text{``order a drink''}\}$. By assumption, we the object of a decision problem is to select a \emph{single} decision from the available set. Suppose I'd prefer to drink water than anything else, and I also think $d_2$ has some chance resulting in not-water. Then I will choose $d_1$ and I will \emph{not} choose $d_2$. At the same time, if I order water it will be true that I ordered a drink. 

Note that if $\mathscr{D}$ is countable, the map $\kernel{T}:\mathscr{D}\to \Delta(\sigalg{O})$ along with $\RV{D}:\mathscr{D}\times O\to \mathscr{D}$ and $\RV{O}:\mathscr{D}\times O\to O$ is a see-do forecast.
