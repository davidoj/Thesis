...
Following Wald's work on statistical decision theory, \citet{savage_foundations_1972} proved a representation theorem for a certain type of decision problem. While Savage was interested in the justification for why statistical decision problems are formalised they way they were by Wald \citep{savage_theory_1951}, the model type representing statistical decision problems discussed above is somewhat different to the model type that is the subject of Savage's representation theorem. In particular, Savage's model type featured a single probability measure over hypotheses (akin to a prior as discussed in the previous chapter) and for decisions considered the set of all functions from hypotheses to consequences.  




 takes as a given that uncertainty in consequences $Y$ is represented by probability and (effectively) that there is a one-to-one function from $D$ to $\Delta(\sigalg{Y})$. Given these assumptions,   \citet{savage_foundations_1972} 
