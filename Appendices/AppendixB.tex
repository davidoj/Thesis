...
Given a kernel $\kernel{K}:D^{\mathbb{N}}\kto Y^{\mathbb{N}}$, let $(H,\sigalg{H}):=\mathcal{M}_1(Y^D)$ be the set of probability distributions on $(Y^D,\sigalg{Y}^D)$. $\kernel{K}$ is IO contractible if and only if there is some $\mu\in \Delta(H)$ and $\kernel{L}:H\times X\kto Y$ such that 
\begin{align}
...
If:
By assumption, for any $\{A_i\in \sigalg{Y}|i\in\mathbb{N}\}$, $d:=(x_i)_{i\in\mathbb{N}}\in D^{\mathbb{N}}$
\begin{align}
...
\begin{align}
    \kernel{K}(A_S|d) &= \int_H \prod_{i\in S}\kernel{L}(A_i|h,x_i)\mu(\mathrm{d}h)\\
