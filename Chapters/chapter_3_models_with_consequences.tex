...
Our task is to find a \emph{model} that relates choice $C$ to consequences $\Omega$. We assume two forms of model -- a \emph{choices only model} associates each choice with a unique probability distribution, and a \emph{choices and hypotheses model} associates each choice with a set of probability distributions. A model consisting of a single probability distribution is also sometimes called a ``Bayesian'' model, so we could call choices only models ``Bayesian'' and choices and hypotheses models ``non-Bayesian''.
...
A decision procedure is a collection $\{\proc{S}_\alpha\}_{\alpha\in C}$ of measurement procedures. We call $C$ the \emph{option set}.
\end{definition}

Like measurement procedures, a decision procedure $\{\proc{S}_\alpha\}_{\alpha\in A}$ isn't a well-defined mathematical object; it's not really a ``set'', because the contents are real-world actions. We assume that each measurement procedure $\proc{S}_\alpha$ yields a value in the same sample space $\Omega$. As a result, each variable $\RV{X}:\Omega\to X$ associates via $\proc{S}_\alpha$ with a subprocedure $\proc{X}_\alpha$. Note that this setup does not guarantee that $\proc{X}_\alpha$ is the same ``kind of measurement'' as $\proc{X}_{\alpha'}$ and so a variable $\RV{X}$ doesn't \emph{necessarily} correspond to a unique kind of measurement. However, it's often a good idea to make sure each variable does correspond to a unique kind of measurement under each available option.

A decision procedure is modelled by a decision model, which assigns a probability measure to each available option, all defined on a common sample space.

\begin{definition}[Decision model]\label{def:dec_model}
A decision model is a triple $(\prob{P}_C, (\Omega,\sigalg{F}), C)$ where $\prob{P}_C$ is the probability set $\{\prob{P}_\alpha|\alpha\in C\}$, $(\Omega,\sigalg{F})$ is the sample space and $C$ is the option set.
\end{definition}
