...
We will now develop a formal result, Theorem \ref{th:det_obs_to_cons}, motivated by Example \ref{ex:doctor_precedent}. The above remarks apply to the formal result as well: if the conditions hold, a decision maker can infer the consequences of their actions from prior knowledge of the effect on an ``input'' variable alone, \emph{without} any further need for understanding how their available options correspond to interventions. 
...
\begin{theorem}[Unobserved to observable IO contractibility]\label{th:latent_to_observable}
Given a CIIR see-do model $(\prob{P}_\cdot,(\RV{E}_i,\RV{X}_i,\RV{Y}_i,\RV{Z}_i)_{i\in\mathbb{N}\cup\{c\}})$ with $E,X,Y$ and $Z$ all discrete, recall $\RV{H}$ is the directing random conditional of $(\prob{P}_\cdot,\RV{Z}_{\mathbb{N}},(\RV{E}_i,\RV{X}_i,\RV{Y}_i)_{i\in \mathbb{N}})$.
