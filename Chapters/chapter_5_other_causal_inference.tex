...
\subsection{Justifying identifiability assumptions}

Theorem \ref{th:potential_outcomes_identifiability} comes with many conditions. The infinite support condition is familiar, and allows us to identify an observed variable that corresponds to ``the true distribution of $\RV{Y}_i$ given $(\RV{X}_i,\RV{D}_i)$. The exchangeable domination condition is used in Theorem \ref{lem:exch_prod_ciid}, and it is likely stronger than necessary.

We will focus on the remaining two assumptions. First, the assumption that $\prob{P}_\alpha^{\RV{Y}^D\RV{X}}$ is exchangeable for all $\alpha$, and secondly the assumption that is sometimes called ``ignorability'': $\RV{D} \CI^e_{\prob{P}_C} \RV{Y}^D | (\RV{X},\RV{Y},\text{id}_C)$. What justifies these assumptions?
