...
\begin{align}
    ...
    &= (\kernel{L}\utimes\kernel{M})\utimes\kernel{N}
\end{align}
This follows directly from Equation \eqref{eq:ccom1}.
\end{proof}
...
For \emph{this} diagram, properties \eqref{eq:labels_express_marginal_upper} and \eqref{eq:labels_express_marginal_lower} do not hold, even though \eqref{eq:labels_express_joint} does.
...
\end{align}
$\kernel{M}$ is a version of $\prob{P}^{\RV{Y}|\RV{X}}$, ``the probability of $\RV{Y}$ given $\RV{X}$''. Let $\prob{P}^{\{\RV{Y}|\RV{X}\}}$ be the set of all kernels that satisfy \eqref{eq:ordinary_disint} and $\prob{P}^{\RV{Y}|\RV{X}}$ an arbitrary member of $\prob{P}^{\RV{Y}|\RV{X}}$.
...
Write $\kernel{K}^{\{\RV{Y}|\RV{XD}\}}$ for the set of kernels satisfying \eqref{eq:def_k_disint} and $\kernel{K}^{\RV{Y}|\RV{XD}}$ for an arbitrary member of $\kernel{K}^{\{\RV{Y}|\RV{XD}\}}$.
\end{definition}
...
and hence the output labels are valid. Diagram \eqref{eq:const_from_m} is constructed by taking the product of the copy map with $\kernel{M}\otimes\textbf{Id}$. Thus it is valid to label $\kernel{M}$ with
...
Thus $Q^{\{0\}}$ is not measurable and consequently $Q$ fails to be a Markov kernel. The problem comes from the fact that $Q$ is defined by an uncountable collection of disintegrations $Q(d)$, each of which is individually measurable. In this case, the problem can be easily solved by defining $Q'$ without the non-measurable component in \eqref{eq:non_measurable_disint}. What we would like are general conditions under which we know that we can choose an appropriate set of disintegrations $Q(d)$ in order for the resulting $Q$ to be a Markov kernel.
...
For a more specific example with Causal Bayesian Networks, suppose we have $\RV{X}$ and $\RV{Y}$ in $[0,1]$, the graph $\mathcal{G}:=\RV{X}\rightarrow \RV{Y}$ and, as above, the observational distribution is $\prob{P}(\RV{X}\in A,\RV{Y}\in B)=\lambda(A)\lambda(B)$. Then, because elements of $\prob{P}^{\{\RV{Y}|\RV{X}\}}$ are unique up to a set of $\kernel{P}$-measure 0, Equation \eqref{eq:non_measurable_disint} is a version of $\prob{P}^{\{\RV{Y}|\RV{X}\}}$ for each $d\in D$. Identify each $d\in D$ with an intervention $do(\RV{X}=x)$. According to the definition of \citet{pearl_causality:_2009} pg 24, the interventional distribution $\prob{P}(d)(\RV{X},\RV{Y})$ must have the following properties for evey $x\in X$:
...
In general, diagram labels are ``well behaved'' with regard to the application of any of the special Markov kernels: identities \eqref{eq:identity}, swaps \eqref{eq:swap}, discards \eqref{eq:discard} and copies \eqref{eq:copy} as well as with respect to the coherence theorem of the CD category. They are not ``well behaved'' with respect to composition.
...
\end{itemize}
\eqref{eq:splitter_preserves_name}: Suppose $\kernel{K}$ is a version of $\prob{P}_{\RV{Y}|\RV{Z}}$. Then
\begin{align}
