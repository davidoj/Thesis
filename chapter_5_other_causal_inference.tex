...
\begin{theorem}[causal contractibility of CBNs]\label{th:causal_contractibility_cbn}
...
It's not obvious how to formalise this general idea of preemption. One possibility, slightly different to the idea of ``unknown exchangeable subsets'' from Theorem \ref{th:condit_exchange}, is to model the data as if the observations were generated by someone much like the decision maker except with a larger set of options available and whose actions are unknown to the decision maker. Specifically, we suppose that our hypothetical ``decision maker analogue'' has a causally contractible model comprising the full sequence of their own choices plus the real decision maker's choices, and they take some unknown sequence of mixed choices. We furthermore assume that they do this in a manner that puts positive probability on taking any of the decision maker's actual choices at each point in time (that is, they didn't just have the \emph{option} to preempt the decision make, but they actually did preempt them). For convenience, we index the variables under the control of hypothetical decision maker analogue with  $i=...,-2,-1$ and the variables under the control of the decision maker with the single index $i=0$. This reflects the intuitive fact that the passive observations/hypothetical decision maker's choices usually come before the consequences of the real decision maker's actions, though in the present analysis the reversed indexing does not play any important role.

\begin{definition}[Preemption]\label{def:preemption}
Given a probability set $\prob{P}_C$ on $(\Omega,\sigalg{F})$ and variables $\RV{Y}:=(\RV{Y}_i)_{-i\in \{0\}\cup\mathbb{N}}$, we say for each $\alpha$, $\prob{P}_\alpha^{\RV{Y}_0}$ is preempted by $\prob{P}_\alpha^{\RV{Y}_{-\mathbb{N}}}$ if there is some $\RV{D}:=(\RV{D}_i)_{-i\in \{0\}\cup\mathbb{N}}$, $\RV{D}_i:\Omega\to D$ for some denumerable $D\supset C$, such that $\RV{D}_0=\RV{C}$ almost surely and and $\prob{P}_\alpha^{\RV{Y}|\RV{WD}}$ is causally contractible over some $\RV{W}$ and, defining $\RV{H}$ to be the directing random measure with respect to $(\RV{P}_C,\RV{D},\RV{Y})$, $\prob{P}_\alpha^{\RV{D}_i}(\alpha)>0$ for all $\alpha\in C$, $i<0$.
\end{definition}

We can consider preemption along with an informal assumption that $\RV{D}$ is unobserved (with the exception of $\RV{D}_0$), see Section \ref{sec:on_unobservability} for a short discussion of this.

The assumption of preemption as given in Definition \ref{def:preemption} can, under some conditions, yield causal conclusions. Theorem \ref{th:det_obs_to_cons} shows that, if $\prob{P}_\alpha^{\RV{Y}_0}$ is preempted by $\prob{P}_\alpha^{\RV{Y}_{-\mathbb{N}}}$ and there is some $(\RV{X}_i,\RV{Z}_i)=f\circ \RV{Y}_i$ such that $\prob{P}_\alpha^{\RV{Z}_i|\RV{W}\RV{X}_i}$ is deterministic in the appropriate sense for $i<0$, then $\prob{P}_\alpha^{\RV{Y}|\RV{WX}}$ will also be causally contractible over $\RV{W}$. This is not often going to be a practically useful theorem -- for many models, the conclusion of determinism will be impossible from finite data, and even if it is possible it will often be rejected. It only shows that it is possible to reason from an assumption of preemption to a causal conclusion.

In the following theorem, consider $\RV{D}$ to be some ``unobserved'' variable and $\RV{X}$ to be an ``observed'' variable.

\begin{theorem}\label{th:det_obs_to_cons}
Suppose, given a probability set $\prob{P}_C$ on $(\Omega,\sigalg{F})$ with variables $\RV{Y}:=(\RV{Y}_i)_{-i\in \{0\}\cup\mathbb{N}}$, $\RV{D}:=(\RV{D}_i)_{-i\in \{0\}\cup\mathbb{N}}$ and for each $i$, $(\RV{X}_i,\RV{Z}_i) = f\circ \RV{Y}_i$ for some $f$ with both $D$ and $X$ denumerable, $\prob{P}_\alpha^{\RV{Y}_0}$ is preempted by $\prob{P}_\alpha^{\RV{Y}_{-\mathbb{N}}}$ with respect to $\RV{D}$ and and some $\RV{W}$. Suppose further that for all $w\in W$, $i<0$ there is some $f_w:D\to Y$ such that
\begin{align}
    \prob{P}_\alpha^{\RV{Z}_i|\RV{W}\RV{X}_i}(A|w,x) &\overset{\prob{P}_\alpha^{\RV{WX}_i}}{\cong} \delta_{f_w(x)}(A)
\end{align}
then $\prob{P}_\alpha^{\RV{Z}|\RV{WX}}$ is also causally contractible over $\RV{W}$.
\end{theorem}

\begin{proof}
Fix $w\in W$. By determinism of $\prob{P}_\alpha^{\RV{Z}_{-1}|\RV{W}\RV{X}_{-1}}$, $\RV{Z}_{-1}\CI^e_{\prob{P}_C} \RV{D}_{-1}|(\RV{W},\RV{Z}_{-1},\RV{C})$. By causal contractibility of $\prob{P}_\alpha^{\RV{Y}|\RV{WD}}$, we also have
\begin{align}
    \prob{P}_\alpha^{\RV{Z}_0|\RV{W}\RV{X}_0\RV{D}_0} &\overset{\prob{P}_\alpha^{\RV{WX}_{-1}\RV{D}_{-1}}}{\cong} \prob{P}_\alpha^{\RV{Y}_{-1}|\RV{W}\RV{X}_{-1}\RV{D}_{-1}}
\end{align}
Note that the assumption $\prob{P}_\alpha^{\RV{D}_{-1}}(\alpha)>0$ for all $\alpha\in C$ implies $\prob{P}_\alpha^{\RV{WX}_{-1}\RV{D}_{-1}}\gg \prob{P}_\alpha^{\RV{WX}_0\RV{D}_0}$ (because the support of $\RV{D}_0$ is limited to $C$), so
\begin{align}
    \prob{P}_\alpha^{\RV{Z}_0|\RV{W}\RV{X}_0}\otimes \mathrm{del}_D &\overset{\prob{P}_\alpha^{\RV{WX}_{-1}\RV{D}_{-1}}}{\cong} \prob{P}_\alpha^{\RV{Z}_{-1}|\RV{W}\RV{X}_{-1}}\otimes \mathrm{del}_D\\
    \implies \prob{P}_\alpha^{\RV{Z}_0|\RV{W}\RV{X}_0}\otimes \mathrm{del}_D &\overset{\prob{P}_\alpha^{\RV{WX}_0\RV{D}_0}}{\cong} \prob{P}_\alpha^{\RV{Z}_{-1}|\RV{W}\RV{X}_{-1}}\otimes \mathrm{del}_D\\
    \implies \prob{P}_\alpha^{\RV{Z}_0|\RV{W}\RV{X}_0} &\overset{\prob{P}_\alpha^{\RV{W}\RV{D}_0}}{\cong}\prob{P}_\alpha^{\RV{Z}_{-1}|\RV{W}\RV{X}_{-1}}\\
    \implies \prob{P}_\alpha^{\RV{Z}_0|\RV{W}\RV{X}_0}(A|w,x) &\overset{\prob{P}_\alpha^{\RV{W}\RV{X}_0}}{\cong} \delta_{f_w(x)}(A)
\end{align}
By determinism, we also have $\RV{Z}_i\CI^e_{\prob{P}_C} (\RV{Z}_{\{i\}^C},\RV{X}_{\{i\}^C})|(\RV{W},\RV{X}_i,\RV{C})$ and so $\prob{P}_\alpha^{\RV{Z}|\RV{WX}}$ is causally contractible over $\RV{W}$.
\end{proof}
...
\end{align}
and if for all $i$, $\RV{Z}_i\CI^e_{\prob{P}_C} \RV{V}_i|(\RV{X}_i,\RV{H},\RV{C})$ then $\prob{P}_\alpha^{\RV{Z}|\RV{HX}}$ is causally contractible over $\RV{H}$.
\end{theorem}
...
\end{align}
Furthermore, by assumption $\RV{Z}_0\CI^e_{\prob{P}_C} (\RV{Z}_{\{0\}^C},\RV{D}_{\{0\}^C}|(\RV{H},\RV{D}_0,\RV{X}_0,\RV{C})$ and $\RV{Z}_0\CI^e_{\prob{P}_C} \RV{D}_{0}|(\RV{H},\RV{D}_0,\RV{C})$ by Equation \ref{eq:indep_of_d}, thus $\prob{P}_\alpha^{\RV{Z}|\RV{HX}}$ is causally contractible over $\RV{H}$.
\end{proof}
...
\subsection{References to individual-level causal contractibility}
