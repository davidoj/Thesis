...
To formalise this, we say there is a variable $\RV{D}$ that is like an ``action'' at the $i=0$ index in the sense that a choice $\alpha$ that leads to a distribution of actions $\RV{D}_0$ identical to some mixture of other choices $\alpha'$ and $\alpha''$ induces consequences equal to the same mixture of $\alpha'$ and $\alpha''$. We require that the model is causally contractible over the sequence of actions $(\RV{D}_i)_{-i\in \{0\}\cup\mathbb{N}}$, and that each action has positive probability in the ``historical'' indices $i<0$. The reason that this assumption is weaker than causal contractibility is that $\RV{D}_i$ for $i<0$ is unobserved -- that is, rules for choosing the action $\RV{D}_0$ cannot depend on $\RV{D}_i$ for $i<0$.
...
\end{align}
and, defining $\RV{H}$ to be the directing random measure of $(\RV{P}_C,\RV{D},\RV{Y})$, $\prob{P}_\alpha^{\RV{D}_i|\RV{H}}(\alpha)>0$ for all $\alpha\in C$, $i<0$.
\end{definition}
...
This proposition might seem obvious here, but precise dependences can arise if we are thinking about the conditional probabilities associated with arbitrary collections of variables. 

\begin{theorem}\label{th:det_obs_to_cons}
Suppose we have a probability set $\prob{P}_C$ on $(\Omega,\sigalg{F})$ with variables $\RV{Y}:=(\RV{Y}_i)_{-i\in \{0\}\cup\mathbb{N}}$, $\RV{D}:=(\RV{D}_i)_{-i\in \{0\}\cup\mathbb{N}}$, $\RV{X}:=(\RV{X}_i)_{-i\in \{0\}\cup\mathbb{N}}$ and $\RV{Z}_i:= (\RV{Z}_i)_{-i\in \mathbb{N}}$, with $D,X,Y,Z$ discrete. Suppose further that $(\prob{P}_C,(\RV{D},\RV{Z}),(\RV{X},\RV{Y}))$ is preempted and $\prob{P}_\alpha^{\RV{Z}\RV{D}_{-\mathbb{N}}}$ exchangeable . Let $\RV{E}$ be the directing random measure of $(\RV{P}_C,\RV{Z},\RV{D})$, $\RV{F}$ the directing random measure of $(\RV{P}_C,(\RV{D},\RV{Z}),\RV{X})$, $\RV{G}$ be the directing random measure of $(\prob{P}_C,(\RV{D},\RV{Z},\RV{X}),\RV{Y})$ and $\RV{H}$ the directing random measure of $(\prob{P}_C,(\RV{D},\RV{Z}),(\RV{X},\RV{Y}))$. Suppose for all $i$, $\RV{Y}_i\CI^e_{\prob{P}_C} \RV{Z}_i|(\RV{H},\RV{X}_i,\RV{C})$. Suppose $\RV{D}_i\not\CI^e_{\prob{P}_C}\RV{Z}_i|(\RV{E},\RV{C})$ or $\RV{X}_i\not\CI^e_{\prob{P}_C}|\RV{Z}_i|(\RV{F},\RV{D}_i,\RV{C})$ and finally suppose for each $f\in F$ $g\in G$, $\alpha\in C$
\begin{align}
    \prob{P}_\alpha^{\RV{F}|\RV{E}}(\cdot|e) \ll U_{[0,1]}\\
    \prob{P}_\alpha^{\RV{G}|\RV{EF}}(\cdot|e,f) \ll U_{[0,1]}
\end{align}
...
\begin{proof}
Because $D,X,Y,Z$ are all discrete, we can identify the directing random measures $\RV{F}$, $\RV{G}$ and $\RV{H}$ with random vectors. In particular, identify the set $F$ with the set of $|Z||D|$-dimensional vectors $(f^d_z)_{d\in D,z\in Z}$, $G$ with the set of $|D||X||Y|$-dimensional vectors $(g^{xy}_d)_{x\in X,y\in Y,d\in D}$ and $H$ with the set of $|Y||D||Z||X|$-dimensional vectors $(h^y_{d,z,x})_{y\in Y,d\in D,z\in Z,x\in X}$.

The assumption $\RV{Y}_i\CI^e_{\prob{P}_C} \RV{Z}_i|(\RV{H},\RV{X}_i,\RV{C})$ implies, for all $h,y,\alpha,z,z',x$ there's some $e, f, g$ such that
\begin{align}
    &\prob{P}_\alpha^{\RV{Y}_i|\RV{H}\RV{Z}_i\RV{X}_i}(y|h,z,x) \overset{\prob{P}_\alpha}{\cong} \prob{P}_\alpha^{\RV{Y}_i|\RV{H}\RV{Z}_i\RV{X}_i}(y|h,z',x)\\
    \implies &\sum_{d\in D} \prob{P}_\alpha^{\RV{Y}_i|\RV{G}\RV{D}_i\RV{Z}_i\RV{X}_i}(y|g,d,z,x)\frac{\prob{P}_\alpha^{\RV{X}_i|\RV{F}\RV{D}_i\RV{Z}_i}(x|f,d,z)\prob{P}_\alpha^{\RV{D}_i|\RV{E}\RV{Z}_i}(d|e,z)}{\sum_{d'\in D}\prob{P}_\alpha^{\RV{X}_i|\RV{F}\RV{D}_i\RV{Z}_i}(x|f,d',z)\prob{P}_\alpha^{\RV{D}_i|\RV{E}\RV{Z}_i}(d'|e,z)}\\
    &\overset{\prob{P}_\alpha}{\cong} \sum_{d\in D} \prob{P}_\alpha^{\RV{Y}_i|\RV{G}\RV{D}_i\RV{Z}_i\RV{X}_i}(y|g,d,z',x)\frac{\prob{P}_\alpha^{\RV{X}_i|\RV{F}\RV{D}_i\RV{Z}_i}(x|f,d,z')\prob{P}_\alpha^{\RV{D}_i|\RV{E}\RV{Z}_i}(d|e,z')}{\sum_{d'\in D}\prob{P}_\alpha^{\RV{X}_i|\RV{F}\RV{D}_i\RV{Z}_i}(x|f,d',z')\prob{P}_\alpha^{\RV{D}_i|\RV{E}\RV{Z}_i}(d'|e,z')}\label{eq:from_ciir_indep_lemma}
\end{align}

Where Equation \ref{eq:from_ciir_indep_lemma} uses \todo[inline]{independence lemma} to replace $\RV{H}$ with the more specific directing measures. We can substantially simplify this by noting that 
\begin{align}
    \prob{P}_\alpha^{\RV{Y}_i|\RV{G}\RV{D}_i\RV{Z}_i\RV{X}_i}(y|g,d,z,x)&= g^y_{dzx}\\
    \prob{P}_\alpha^{\RV{X}_i|\RV{F}\RV{D}_i\RV{Z}_i}(x|f,d,z)&=f^x_{dz}\\
    \prob{P}_\alpha^{\RV{D}_i|\RV{E}\RV{Z}_i}(d|e,z)&=e^d_z
\end{align}
then, rewriting Equation \ref{eq:from_ciir_indep_lemma}
\begin{align}
    \sum_{d\in D} g^y_{dzx}\frac{f^x_{dz}e^d_z}{\sum_{d'\in D}f^x_{d'z}e^{d'}_z}&\overset{\prob{P}_\alpha}{\cong} \sum_{d\in D} g^y_{dzx}\frac{f^x_{dz'}e^d_{z'}}{\sum_{d'\in D}f^x_{d'z'}e^{d'}_{z'}}
\end{align}
