...
\section{Assessing decision problems for causal contractibility}

Causal contractibility is a condition that, if it holds, allows an analyst to reason as if there is a fixed probabilistic map $D\kto Y$ representing the relation between all pairs of ``$\RV{D}_i$-type'' variables and corresponding ``$\RV{Y}_i$-type'' variables, no matter what choice is selected. The question is: when should an analyst, confronted with a decision problem actually adopt a causally contractible model $\prob{P}_C$ to help them make their decision? This is not an easy question for several reasons. Two of these are:
\begin{itemize}
