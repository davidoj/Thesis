...
\begin{corollary}\label{cor:extend_to_randomised}
Given a sequential just-do model $(\prob{P}_{C'},(\RV{D},\RV{I}),\RV{Y})$ satisfying the conditions of Theorem \ref{th:cc_ind_treat} and a second model $(\prob{P}_{C},(\RV{D},\RV{I}),\RV{Y})$ such that for all $\alpha\in C$ there is some probability measure $\mu\in \Delta(C')$ such that
\begin{align}
    \prob{P}_\alpha &= \sum_{\beta\in C'} \mu(\beta) \prob{P}_\beta 
\end{align}
...
For all $\alpha\in C$
\begin{align}
    \prob{P}_\alpha^{\RV{Y}|\RV{D}} &= \sum_{c\in C'} \mu(\beta) \prob{P}_\beta^{\RV{Y}|\RV{D}}\\
    &= \prob{P}_{C'}^{\RV{Y}|\RV{D}}
\end{align}
\end{proof}

Corollary \ref{cor:extend_to_randomised} can be used to argue that a randomised experiment of the right type is causally contractible. The following example is similar to the setup in \citet{kasy_why_2016}. In this, Kasy argues that as long as the experimenter controls the treatment assignment, causal effects are identified (i.e. the randomisation step is not strictly necessary). We largely agree; however, Kasy's argument depends on the assumption that the choice (i.e. the \emph{output} of the decision-making step) corresponds deterministically and 1-to-1 with the sequence of treatment assignments. This is a natural assumption to make when analysing experimental design as a decision problem, but in practice this is not how many experiments are conducted. Example \ref{ex:randomised_experiment} replicates Kasy's argument with our notation, while Example \ref{ex:bad_randomised_experiment} shows how things can go wrong if this assumption is not satisfied.
...
A sequential experiment is modeled by a probability set $\prob{P}_C$ with treatment functions $\RV{D}:=(\RV{D}_i)_{i\in [N]}$, treatments $\RV{T}:=(\RV{T}_i)_{i\in [N]}$ covariates $\RV{X}:=(\RV{X}_i)_{i\in [N]}$ and outcomes $\RV{Y}:=(\RV{Y}_i)_{i\in[N]}$. Suppose this is extendable to a model with infinite sequences $(\RV{T}_i)_{i\in \mathbb{N}}$, covariates $(\RV{X}_i)_{i\in \mathbb{N}}$ and outcomes $(\RV{Y}_i)_{i\in \mathbb{N}}$ where $i>N$ means $\RV{T}_i$ is unobserved. The set of choices $C$ is the set of all probability distributions$\Delta(D^N)$ for some $N\subset\mathbb{N}$ (this is to ensure $C$ is countable). Let $C'\subset C$ be the deterministic elements of $C$.

Each treatment $\RV{T}_i$ is given to a patient, and each patient provides a unique identifier $\RV{I}_i$ which for simplicity we assume is a number in $\mathbb{N}$ (instead of, say, a driver's license number and state of issue), and that (implausibly) there is a positive probability for $\RV{I}_i$ to take any value in $\mathbb{N}$ for any choice $\alpha$. The mechanism by which they received these identifiers is not known, and they are judged to offer no evidence regarding the treatment function, the administration of treatment or the outcomes of treatments. That is, $\RV{DXY}\CI^e_{\prob{P}_C} \RV{I}|\RV{C}$ (if each identifier is unique, this is equivalent to assuming that given any choice $\alpha$, the model is unchanged by permutation of identifiers -- see also Lemma \ref{lem:ind}).

The experimental setup is: the analyst consults the model $\prob{P}_C$, and accordingly decides on possibly stochastic sequence $\alpha:=(\mu_i)_{i\in N}$ of decision functions $X\kto Y$. For $i\leq N$, if $\mu_i$ is deterministic with probability 1 on some $d_i$, the experimental administrator will assign patient $i$ a treatment $\RV{T}_i$ according to $d_i(\RV{X}_i)$. Otherwise, if it is stochastic, the administrator will consult a random number generator and apply some fixed rule dependent on the required probabilities in order to choose the appropriate decision function, then assign the treatment according to that function. For $i>N$, the administrator chooses a decision function with uniform probability, again according to the random number generator. Beyond specifying the decision function, the choice is deemed irrelevant to any of the outcomes, and beyond specifying the covariate-treatment relationship the decision functions are also deemed irrelevant to the outcomes - i.e. $\RV{YIXT}\CI^e_{\prob{P}_C} \RV{C}|\RV{D}$ and $\RV{YI}\CI^e_{\prob{P}_C} (\RV{D},\RV{C})|(\RV{X},\RV{T})$.

The randomisation procedure is deemed sufficient to ensure that for any mixed $\alpha\in C$ where $\alpha = \sum_{\beta\in C'} \mu(\beta) \beta$, $\prob{P}_\alpha = \sum_{\beta\in C'} \prob{P}_\beta$.

Furthermore, the treatment function $\RV{D}_i$ and the identifier $\RV{I}_i$ are judged irrelevant to the outcomes of any patients other than $i$; that is, $\prob{P}_C^{\RV{YXTI}|\RV{DI}}$ is local, Definition \ref{def:caus_cont}. and given any subsequence of patients, covariates and treatments the same joint distribution over consequences is expected no matter where in the experiment these patients appeared. That is, for any $A,B\subset\mathbb{N}$, $|A|=|B|$
\begin{align}
    \prob{P}_C^{\RV{Y}_A\RV{X}_A\RV{T}_A|\RV{D}_A\RV{I}_A} &\overset{\prob{P}_C}{\cong} \prob{P}_C^{\RV{Y}_B\RV{X}_B\RV{T}_B|\RV{D}_B\RV{I}_B}
\end{align}
Together, these assumptions imply $\prob{P}_C^{\RV{Y}|\RV{XTI}}$ is causally contractible, see Theorem \ref{th:equal_of_condits}.

For the deterministic subset $C'\subset C$, $\RV{YI}\CI^e_{\prob{P}_{C'}} \RV{D}|\RV{C}$ as $\RV{D}$ is deterministic for all elements of $C'$. By application of Theorem \ref{th:cc_ind_treat}, $\prob{P}_{C'}^{\RV{YXT}|\RV{D}}$ is causally contractible, and by application of Corollary \ref{cor:extend_to_randomised}, so is $\prob{P}_{C}^{\RV{YXT}|\RV{D}}$

By taking higher order conditionals, it follows that $\prob{P}_C^{\RV{Y}|\RV{XTD}}$ is also causally contractible, and by Lemma \ref{lem:ind_to_cc} applied with $\RV{YI}\CI^e_{\prob{P}_C} (\RV{D},\RV{C})|(\RV{X},\RV{T})$, $\prob{P}_C^{\RV{Y}|\RV{XT}}$ is also causally contractible. If, furthermore, the covariates $\RV{X}$ are exchangeable and unaffected by the choice $\RV{X}\CI^e_{\prob{P}_C} \RV{C}$, $\prob{P}_C^{\RV{Y}|\RV{T}}$ is also causally contractible.
\end{example}

Example \ref{ex:randomised_experiment} is causally contractible. However, changing it in a way that might seem minor from some points of view undermines this result.

\begin{example}\label{ex:bad_randomised_experiment}
The setup is precisely as in Example \ref{ex:randomised_experiment}, except given some nondeterministic $\alpha\in C$ the administrator will enact an ad-hoc ``randomisation'' procedure, while given a deterministic choice, the administrator will still assign treatment deterministically according to the functions given.

By leaving the details of the randomisation up to the administrator, the analyst now has to contend with the fact that the patients may not be as indistinguishable to the administrator as they were to the analyst when constructing the model. For example, the administrator might examine the patients prior to selecting a decision function. If the administrator is not aware of the importance of patient indistinguishability, they may prefer to treat patients who they correctly anticipate will benefit. This means that the analyst must at least consider the possibility that for non-deterministic choices $\alpha$, $\prob{P}_\alpha^{\RV{Y}|\RV{T}}$ will converge to stronger correlation between $\RV{Y}$ and $\RV{T}$ than $\prob{P}_\beta^{\RV{Y}|\RV{T}}$ will for deterministic $\beta$ where the administrator has no discretion. That is, $\prob{P}_\alpha^{\RV{Y}|\RV{T}}\neq \prob{P}_\beta^{\RV{Y}|\RV{T}}$ and so there isn't even a uniform conditional distribution $\prob{P}_C^{\RV{Y}|\RV{T}}$, so there is no question of causal contractibility.
\end{example}
