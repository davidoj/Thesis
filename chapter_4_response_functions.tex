...
\begin{proof}
$\RV{Y}^D$ is a function of $\RV{Y}$ and $\RV{D}$ (see Definition \ref{def:tab_cd}) and $\RV{H}$ is a function of $\RV{Y}^D$. Say $f:Y\times D\to H$ is such that $\RV{H}=f(\RV{Y},\RV{D})$ (see Definition \ref{def:dir_rand_meas}). Because $\RV{H}=f(\RV{Y},\RV{D})$, we have $\RV{H}\CI^e_{\prob{P}_C} (\RV{W},\RV{C})|(\RV{Y},\RV{D})$. Thus
\begin{align}
...
\begin{align}
    \prob{P}_\alpha^{\RV{Y}^D\RV{H}|\RV{W}} &= \tikzfig{p_yd_h_on_w} \\
...
\end{align}
From Theorem \ref{th:table_rep_kernel} we also have $\RV{Y}^D\CI^e_{\prob{P}_C} \RV{W}|(\RV{H},\RV{C})$ , so
\begin{align}
