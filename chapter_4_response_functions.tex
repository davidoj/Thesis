...
\begin{proof}
The map $\kernel{Q}:(B|i,\alpha)\mapsto \prob{P}_\alpha^{\RV{Y}|\RV{I}}(B|i)$ is itself a Markov kernel. By assumption on $f$ there is a right inverse $f^\dagger$ such that, for any $\alpha$,
\begin{align}
    \prob{P}_\alpha^{\RV{Y}|\RV{ID}} \overset{\prob{P}_\alpha}{\cong} (\text{id}_{I}\otimes\kernel{F}_{f^\dagger})\kernel{Q}
\end{align}
...
Therefore
\begin{align}
    (\rho \otimes \text{id}_{C} ) \prob{P}_\alpha^{\RV{Y}|\RV{ID}}&= (\rho \otimes \text{id}_{C} ) (\text{id}_{I}\otimes\kernel{F}_{f^\dagger})\kernel{Q}\\
     &=  (\text{id}_{I}\otimes\kernel{F}_{f^\dagger}) (\rho \otimes \text{id}_{C} )\kernel{Q}\\
     &= (\text{id}_{I}\otimes\kernel{F}_{f^\dagger})\kernel{Q}\\
