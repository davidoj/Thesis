...
Chapter \ref{ch:tech_prereq} introduced probability sets as generic tools for causal modelling, while Chapter \ref{ch:2p_statmodels} examined how probability set models can be used in decision problems and Section \ref{sec:cons_to_sdp} in particular introduced \emph{see-do} models, which featured variables representing observations and consequences, choices and hypotheses. So far, no attention has been paid to how exactly anybody might use a see-do model to help learn from observations. Observations and consequences in see-do models could be just about anything -- they need not take values in the same set, nor be related to one another in any particular way. 

As has been pointed out in Section \ref{sec:assumptions}, a distinguishing feature of decision making is the need to compare the consequences of multiple different options. If a decision maker can only make one choice, and never faces a similar situation ever again, then the consequences of every option he didn't choose must be, to some extent, speculative. On the other hand, if the decision maker faces a problem that is essentially the same over and over again, he can choose a different ``input'' at each point in time to learn about the consequences of each. This offers a way for the decision maker to use previous data -- the inputs and consequences previously observed -- to inform his choice of future inputs. The notion that the decision maker is facing ``essentially the same'' problem over and over again is an assumption. In particular, the decision maker is assuming that, whenever he makes the same choice, the same distribution over consequences will ensue. That is, he is assuming that the sequence of problems he is facing are modeled by a sequence of \emph{identical functions} that map choices to distributions over consequences.

As we will see in Chapter \ref{ch:other_causal_frameworks}, this assumption underpins many causal identifiability results in popular causal frameworks. However, evaluating the assumption of repeated functions is tricky. It is not as if the decision maker can inspect the source code of the world and determine that particular sets of inputs are always mapped to particular sets of outputs in a manner predictable up to random variation. It will also be rare that the decision maker knows of any physical laws that imply the required functional regularity. 

Predictive models often employ a very similar structure. Instead of assuming identical response functions, predictive models often assume identically distributed variables. If a sequence of pairs $(\RV{X}_i,\RV{Y}_i)$ are identically distributed, then this implies that the probability of $\RV{Y}_i$ given $\RV{X}_i$ is identical for all $i$, and vise-versa. It has been argued that this assumption can be justified by appealing to a symmetry in the analyst's knowledge about the problem. In particular, 
