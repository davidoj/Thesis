...
\begin{lemma}
Suppose a sequential input-output model $(\prob{P}_C,\RV{D},\RV{Y})$ is such that there is some $\RV{W}$ such that $\RV{Y}\CI^e_{\prob{P}_C} \RV{W}|(\RV{H},\RV{C})$ and for all $\alpha$
\begin{align}
...
\begin{proof}
As $\RV{H}$ is a function of $\RV{Y}$ and $\RV{D}$, we have
\begin{align}
    \prob{P}_\alpha^{\RV{YH}|\RV{WD}} &= 
\end{align}
\end{proof}
