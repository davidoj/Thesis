...
\end{enumerate}
However the reasons given for this are incomplete. Results about the identifiability of potential outcomes such as those in \citet{rubin_causal_2005,imbens_causal_2015} rely on a tacit understanding of the relevance of potential outcomes to consequences of choices, and the same is true for the ``intervention'' operation in causal graphical models (see, for example, \citet[Chap. 4]{pearl_book_2018}). Other arguments (considered in more detail in Section \ref{sec:prev_work}) consider the possibility of physically swapping experimental subjects without altering the relevant model, or the idea that the process that gave rise to our observations is in some sense similar to the process that gives rise to the consequences of our choices, which begs the question: \emph{in what sense} are they similar?

A theory of causal inference seemingly ought to be able to explain these basic facts. Introducing them as \emph{postulates} seems to be a mistake -- it is possible, sometimes, to learn about the likely consequences of choices without randomized experimental data, and furthermore, introducing a postulates depending on randomized experiment demands a rigorous definition of ``randomized experiment''.

The key theorems in this chapter can offer an answer to this question. From Chapter \ref{ch:2p_statmodels}, we have the idea of a probability set $\prob{P}_C$ modelling the consequences of a set of choices $C$. Given such a probability set, \emph{independent and identical response functions} are proposed as the relevant notion of ``causal effect''. Given these two ideas, the question remains: when should a probability set $\prob{P}_C$ be chosen to be a mixture of independent and identical response functions, and in particular is that a good choice for modelling randomized experiments in conjunction with consequences of choices?

Theorem \ref{th:data_ind_CC} says a model $\prob{P}_C$ of the consequences of choices is a mixture of independent and identical response conditionals if and only if it is causally contractible in the appropriate sense. While causal contractibility is a necessary and sufficient condition, it isn't particularly obvious when it is appropriate for a model to be causally contractible in this sense. It is sometimes clear that causal contractible is \emph{not} a reasonable assumption -- see Section \ref{sec:examples}, but less clear when it might be reasonable.

Theorem \ref{th:cc_ind_treat} is a sufficient condition for causal contractibility that formalises the intuition that causal contractibility is appropriate for models of experiments that are, in some sense, ``all alike'' (recall the suggestion of models that are unchanged after physically swapping experimental subjects). This is formalised by the condition that each experiment can be assigned a unique \emph{identifier} such that the model is unchanged by permutation of identifiers. On its own, this condition is not sufficient for causal contractibility of an arbitrary pair of variables $\RV{D}$ and $\RV{Y}$. While the experiments may all look alike at the time the choice is made, learning $\RV{D}_i$ might tell us more about the experiment that generates the $(\RV{D}_i,\RV{Y}_i)$ pair in a way that breaks this symmetry. Alternatively, $\RV{D}_i$ may not capture all the features of a choice $\alpha$ relevant to the consequence $\RV{Y}_i$. Thus an additional requirement for causal contractibility is that the input variables $(\RV{D}_i)_{i\in \mathbb{N}}$ screen off the choice $\alpha$ from the consequences $(\RV{Y}_i)_{i\in\mathbb{N}}$ \emph{as well as the converse}. This is satisfied, for example, if $(\RV{D}_i)_{i\in \mathbb{N}}$ is deterministically an invertible function of the choice $\alpha$. 

Finally, Corollary \ref{cor:extend_to_randomised} extends Theorem \ref{th:cc_ind_treat} to models with randomised inputs, which is enough to propose an answer to the original question: Example \ref{}. 

While Theorem \ref{th:cc_ind_treat} places strong conditions on the distribution of inputs given a choice, randomization itself is not among them.
...
\begin{align}
    ...
    &\overset{\prob{P}_C}{\cong} \tikzfig{index_independence_3}\\
    \implies \prob{P}_C^{\RV{Y}|\RV{D}} &\overset{\prob{P}_C}{\cong} \tikzfig{index_independence_4}
\end{align}
In particular, for $j\in I$ this holds for some $\nu$ such that $\nu_i(j)=1$ for all $i\in \mathbb{N}$. Thus
\begin{align}
    \prob{P}_C^{\RV{Y}|\RV{D}} &\overset{\prob{P}_C}{\cong} \tikzfig{index_independence_5}
\end{align}
...
Corollary \ref{cor:extend_to_randomised} can be used to argue that a randomised experiment of the right type is causally contractible. The following example is similar to the setup in \citet{kasy_why_2016}. There's a bit of awkwardness in the setup needed to assign each experimental run a unique identifier with positive probability of each identifier being assigned to each run under some choice.

\begin{example}\label{ex:randomised_experiment}
A sequential experiment is modeled by a probability set $\prob{P}_C$ with treatment functions $\RV{D}:=(\RV{D}_i)_{i\in [N]}$, treatments $\RV{T}:=(\RV{T}_i)_{i\in [N]}$ covariates $\RV{X}:=(\RV{X}_i)_{i\in [N]}$ and outcomes $\RV{Y}:=(\RV{Y}_i)_{i\in[N]}$. Suppose this is extendable to a model with infinite sequences $(\RV{T}_i)_{i\in \mathbb{N}}$, covariates $(\RV{X}_i)_{i\in \mathbb{N}}$ and outcomes $(\RV{Y}_i)_{i\in \mathbb{N}}$ where $i>N$ means $\RV{T}_i$ is unobserved. The set of choices $C=D^{\mathbb{N}}\times \mathbb{N}$.

Call a tuple $(\RV{D}_i,\RV{T}_i,\RV{X}_i,\RV{Y}_i)$ a \emph{run} of the experiment. Each run $i$ of the experiment is given a unique identifier $\RV{I}_i$ which we can assume without loss of generality is deterministically equal to $i+n$ given the choice $(d,n)\in C$. This way, each run has positive probability of receiving each identifier under some choice.

Assume $\prob{P}_C^{\RV{Y}|\RV{TXI}}$ is causally contractible. That is, 

In particular, each choice 
\end{example}
