...
This chapter introduces \emph{conditionally independent and identical response functions} as the ``something in common'' that a sequence of subprocedures might have. Section \ref{sec:response_functions} defines conditionally independent and identical response functions and explores different examples of models that feature them. The key theorem in this chapter, Theorem \ref{th:ciid_rep_kernel}, relates the structural assumption of conditionally independent and identical response functions to a kind of symmetry which we call \emph{causal contractibility}. A model features conditionally independent and identical response functions if and only if it is causally contractible.
...
Consider the previous example, except instead of watching knowledgable operators, the DM will pull each lever themselves, and they will decide in advance on the sequence of pulls. In this case, the dependence between $\RV{H}$ and $\RV{D}$ is removed and
\begin{align}
...
\begin{itemize}
    \item If $(\prob{P}_C,\RV{D},\RV{Y})$ is causally contractible over $\RV{H}$, when is $(\prob{P}_C,\RV{D}_{(n,\infty)},\RV{Y}_{(n,\infty)})$ approximately causally contractible $(\RV{D}_i,\RV{Y}_i)_{i\in [n]}$ for sufficiently large $n$?
\end{itemize}
In particular, if $(\prob{P}_C,\RV{D}_{(n,\infty)},\RV{Y}_{(n,\infty)})$ is approximately causally contractible over $(\RV{D}_i,\RV{Y}_i)_{i\in [n]}$ for sufficiently large $[n]$, then Equation \ref{eq:interchangeability_of_conditioning} may hold approximately whenever $|A|$ is sufficiently large.

In a more realistic setting where observational data is mixed with experimental data, causal contractibility is seemingly never accepted. For example, \citet{eckles_bias_2021,gordon_comparison_2018,gordon_close_2022} all separate the observational and experimental data and \emph{test} whether the conditional probabilities derived from each appear to be equal, whereas the assumption of causal contractibility implies that one could just as well use the observational data to estimate conditional probabilities for unseen experimental pairs.

Finally, an example of ``backdoor adjustment''. Suppose a sequential input-output model $(\prob{P}_C,(\RV{D},\RV{X}),\RV{Y})$ is causally contractible over some $\RV{W}$, and:
\begin{itemize}
