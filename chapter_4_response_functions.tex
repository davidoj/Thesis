...
\begin{itemize}
    \item For some choice, what are the \emph{likely consequences} with regard to $\RV{Y}_i$?
    \item What is the \emph{relative frequency} of different $(\RV{D}_i,\RV{Y}_i)$ pairs across all the different values of $i$?
\end{itemize}
The first is what the decision maker wants to know in order to make a good decision, and the second is what they can learn as they accumulate data. A decision maker might be particularly happy to if they have good reason to expect that, no matter what choice they make, the likely consequences with regard to $\RV{Y}_i$ for a given value of $\RV{D}_i$ should be a probability distribution that in the long term limit matches the relative frequency of different values of $\RV{Y}_j$ for the same values of $\RV{D}_j$ averaged over $j$. In this situation, a decision maker that already knows how their choices affect $\RV{D}_i$ can use the accumulated data to improve their guess about how $\RV{Y}_i$ will respond to each choice.

A decision maker in this position may choose a model $\prob{P}_C$ with \emph{conditionally independent and identical response functions} relating the $(\RV{D}_i,\RV{Y}_i)$ pairs. In such a model there is some hypothesis $\RV{H}$ such that $\prob{P}_C^{\RV{Y}_i|\RV{D}_i\RV{H}}$ is identical for all values of $i$. This conditional distribution $\prob{P}_C^{\RV{Y}_i|\RV{D}_i\RV{H}}$ represents the ``likely consequences with regard to $\RV{Y}_i$ of some $\RV{D}_i$'' ' and, under some conditions, is a deterministic function of the limiting relative frequencies of the $(\RV{D}_i,\RV{Y}_i)$ pairs (see Theorem \ref{th:data_ind_CC}).

In this chapter, we introduce \emph{causal contractibility} and show it is a key property of decision models with conditionally independent and identical response functions. In particular, a Markov kernel relating two sequences is a mixture of independent and identical response functions if an only if it is causally contractible (Theorem \ref{th:ciid_rep_kernel}). Section \ref{sec:data_independent_actions} shows how the assumption of a causally contractible conditional distribution implies conditionally independent and identical response functions.

One of the implications of of the assumption in Section \ref{sec:data_independent_actions} is that what we call ``inputs'' are independent of all other data pairs conditional on the hypothesis $\RV{H}$. This rules out, for example, a model where inputs are learned from prior experience. Section \ref{sec:data_dependent} removes this assumptions.

A further extension is to consider models where the hypothesis and inputs are dependent. Theorem \ref{th:ciid_rep_kernel_nolocal} addresses one such situation, where the hypothesis depends on a symmetric function of the inputs. This can arise in models where inputs are thought to be chosen in a strategic but unknown way, or when there are ``population saturation effects'' such as herd immunity. More examples of this kind of dependence are addressed in Chapter \ref{ch:other_causal_frameworks}, where we examine potential outcomes models as a case of variable dependence between the hypothesis and the inputs.

\subsection[Causally contractible Markov kernels]{Causally contractible Markov kernels - definitions and explanation}\label{sec:ccontracibility}

The assumptions of \emph{exchange commutativity} and \emph{locality} together make \emph{causal contractibility}. The general result proved in this section is applied to ``data-independent'' decision problems in Section \ref{sec:data_independent_actions} and do ``data-dependent'' problems in Section \ref{sec:data_dependent}.
...
Theorem \ref{th:data_ind_CC} applies Theorem \ref{th:ciid_rep_kernel} to the case of a model with data-independent actions and derives the required conditional independences and equalities to show that a sequential just-do model $(\prob{P}_C,\RV{D},\RV{Y})$ with causally contractible $\prob{P}_C^{\RV{Y}|\RV{D}}$ satisfies the required conditional independences and equalities of conditional distributions for $\RV{D}$ and $\RV{Y}$ to be related by independent and identical response functions. There is a requirement that all values of $D$ appear infinitely often; if this were not present, then in general the measure $\RV{H}$ would not be a function of the observed variables.

\begin{theorem}[Data-independent causal contractibility]\label{th:data_ind_CC}
Given a sequential just-do model $(\prob{P}_C,\RV{D},\RV{Y})$ on $(\Omega,\sigalg{F})$ with $D$ countable, let $E\subset D^{\mathbb{N}}$ be the set of all sequences $d$ such that each $x\in D$ appears in $d$ infinitely often. If for all $\alpha\in C$ $\prob{P}_\alpha^{\RV{D}}(E)=1$, then $\prob{P}_C^{\RV{Y}|\RV{D}}$ is causally contractible if and only if there is some directing random measure $\RV{H}:\Omega\to H$ such that $\prob{P}_C^{\RV{Y}_i|\RV{D}_i\RV{H}}=\prob{P}_C^{\RV{Y}_j|\RV{D}_j\RV{H}}$ for all $i,j\in \mathbb{N}$ and $\RV{H}\CI_{\prob{P}_C} (\RV{D},\RV{C})$.
\end{theorem}
...
\end{align}
That is, the $(i,j)$-th coordinate of $\RV{Y}^D$ is equal to the coordinate $\RV{Y}_k$ for which the corresponding $\RV{D}_k=j$ and there have been $i-1$ preceding instances where a coordinate of $\RV{D}$ has been equal to $j$. Note that $\RV{Y}^D_{ij}$ is equal to some $\RV{Y}_k$ with probability 1, by the assumption that all values of $D$ occur infinitely often almost surely.

Note that $\#_{\RV{D}_k=j}^i$ and $\#_{\RV{D}_l=j}^i$ are mutually exclusive for $k\neq l$. Thus there is a random permutation $\RV{R}$ taking values in the set of functions $\mathbb{N}\to \mathbb{N}$ (including infinite permutations) such that
\begin{align}
    \RV{Y}^D &= (\RV{Y}_{\text{ef}(\RV{R}, i)})\
    := \RV{Y}_{\RV{R}}
\end{align}
where $\text{ef}$ is the function evaluation map $\text{ef}(f,x)=f(x)$. By construction, $\RV{D}_{\RV{R}}\text{ mod }|D|=d_{i\text{ mod }|D|}$ i.e. the $i\text{ mod }|D|$'th element of $D$.

Now,
\begin{align}
    \prob{P}^{\RV{Y}_{\RV{R}}|\RV{D}_{\RV{R}}}_\alpha(A|d) &= \int_R \prob{P}_\alpha^{\RV{Y}_\rho|\RV{D}_{\rho}}(A|d)\prob{P}_\alpha^{\RV{R}}(mathrm{d}\rho)\label{eq:need_ccont}\\
\end{align}
For each $\rho$, define $\rho^n:\mathbb{N}\to \mathbb{N}$ as the permutation that agrees with $\rho$ on the first $n$ indices and is the identity otherwise. By causal contractibility, for $n\in \mathbb{N}$
\begin{align}
...
\begin{align}
    \prob{P}^{\RV{Y}_{\RV{R}}|\RV{D}_{\RV{R}}}_\alpha(A|d) &= \int_R \prob{P}_\alpha^{\RV{Y}_\rho|\RV{D}_{\rho}}(A|d)\prob{P}_\alpha^{\RV{R}}(mathrm{d}\rho)
\end{align}
Define $e:=(e_i)_{i\in\mathbb{N}}$ such that $e_{i+|D|j}$ is the $i$th element of $D$ for all $i,j\in \mathbb{N}$. Then
\begin{align}
    \prob{P}_\alpha^{\RV{Y}^D}(A) &= \prob{P}_C^{\RV{Y}|\RV{D}}(A|e)
\end{align}
By exchange commutativity and invariance of $e_i$ to permutations of period $|D|$, $\prob{P}_\alpha^{\RV{Y}^D}$ is column exchangeable. Thus from \citet[Prop. 1.4]{kallenberg_basic_2005}, there exists a directing random measure $\RV{H}^*:Y^{D\times\mathbb{N}}\to H$ such that, defining $\RV{H}=\RV{H}^*\circ\Pi_{D\times\mathbb{N}}$
\begin{align}
