...
\begin{lemma}[Exchangeable to dominated]\label{lem:exch_to_dom}
\todo[inline]{...}
\end{lemma}
...
If $\prob{P}_C^{\RV{Y}|\RV{D}}$ is exchange commutative, and for each $\alpha$ $\prob{P}_\alpha^{\RV{D}}$ is absolutely continuous with respect to some exchangeable distribution in $\Delta(D^{\mathbb{N}})$ then there is some $\RV{W}$ symmetric over $\RV{D}$ such that $(\prob{P}_C,\RV{D},\RV{Y})$ is IO contractible over $\RV{W}$ and $\RV{Y}_i\CI^e_{\prob{P}_C} \RV{W}|(\RV{D}_i,\RV{H},\RV{C})$.
\end{theorem}
...
Consider $\prob{P}_\alpha^{\RV{Y}_{[n]}|\RV{D}}$ for some $n\in \mathbb{N}$. By assumption of exchange commutativity, for any permutation $\rho_{>n}$ that only affects indices after the $n$th, $\prob{P}_\alpha^{\RV{Y}_{[n]}|\RV{D}}=_\alpha^{\RV{Y}_{[n]}|\RV{D}_{\rho_{>n}}}$. That is, $\prob{P}_\alpha^{\RV{Y}_{[n]}|\RV{D}}$ is $\sigalg{E}_{\RV{D}_(n,\infty)}\vee\sigma(\RV{D}_{[n]}$-measurable.

Let $\sigalg{T}_{\RV{D}}$ be the tail $\sigma$-algebra generated by $\RV{D}$ defined as the intersection $\sigalg{T}_{\RV{D}}:=\cap_{i=1}^{\infty} \sigma(\RV{D}_{[i,\infty)})$. Note that $\sigalg{T}_{\RV{D}_{(n,\infty)}}=\sigalg{T}_{\RV{D}}$. By \citet[Corollary 1.6]{kallenberg_basic_2005} and the assumption that assumption that $\prob{P}_\alpha^{\RV{D}}$ is dominated by an exchangeable distribution, $\sigalg{E}_{\RV{D}_(n,\infty)}=\sigalg{T}_{\RV{D}_{(n,\infty)}}$ almost surely for any $n$. But $\sigalg{T}_{\RV{D}_{(n,\infty)}}=\sigalg{T}_{\RV{D}}$, so $\sigalg{E}_{\RV{D}_(n,\infty)}=\sigalg{T}_{\RV{D}}$ almost surely for any $n$, and thus $\sigalg{E}_{\RV{D}_(n,\infty)}=\sigalg{E}_{\RV{D}}$ almost surely.

Thus $\prob{P}_\alpha^{\RV{Y}_{[n]}|\RV{D}}$ is $\sigalg{E}_{\RV{D}}\vee\sigma(\RV{D}_{[n]})$-measurable. By \citet[Corollary 1.6]{kallenberg_basic_2005} again, there is a random $\RV{J}$ taking values in the set of distributions on $D$ such that $\sigma(\RV{J})=\sigalg{E}_{\RV{D}}$ almost surely. Thus
\begin{align}
...
It remains to be shown that $\RV{Y}_i\CI^e_{\prob{P}_C} \RV{J}|(\RV{D}_i,\RV{H},\RV{C})$. Recall $\prob{P}_\alpha^{\RV{D}}$ is dominated by an exchangeable distribution, call it $\nu$. Because $\prob{P}_C^{\RV{Y}|\RV{D}}$ is exchange commutative, $\nu\prob{P}_C^{\RV{Y}|\RV{D}}$ is also exchangeable, and thus $\prob{P}_\alpha^{\RV{D}}\prob{P}_C^{\RV{Y}|\RV{D}}$ is dominated by an exchangeable distribution.

\todo[inline]{Apply prev lemma}
\end{proof}
...
By construction
\begin{align}
    \prob{Q}_\alpha^{\RV{D}_i\RV{Y}_A\RV{D}_A}\odot \prob{P}_\alpha^{\RV{Y}_i|\RV{D}_i\RV{Y}_A,\RV{D}_A}
\end{align}
is exchangeable.

\todo[inline]{apply prev lem}
