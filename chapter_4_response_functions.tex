...
Start with a sequence of variable pairs $(\RV{X}_i,\RV{Y}_i)_{i\in \mathbb{N}}$ where $\RV{X}_i$ is the $i$th ``input'' and $\RV{Y}_i$ is the corresponding ``output'', each taking values in $X$ and $Y$ respectively. $(\RV{X}_i,\RV{Y}_i)_{i\in \mathbb{N}}$ are related by a sequence of ``independent and identical response functions'' (``IIRs'') if there is some variable $\RV{H}$ (generally a latent variable) such that $\prob{P}_C^{\RV{Y}_i|\RV{X}_i\RV{H}}=\prob{P}_C^{\RV{Y}_j|\RV{X}_j\RV{H}}$ for all $i,j$ and for all $i$ $\RV{Y}_i\CI_{\prob{P}_C}^e (\RV{X}_{<i},\RV{Y}_{<i},\RV{C})|(\RV{H},\RV{X}_i)$.
...
This change gives the administrator a degree of freedom: they can choose some $\alpha$ from the set $C$, and moreover they can potentially do this with access to knowledge that the analyst lacks. Though all identifiers might seem the same to the analyst, they might not seem that way to the administrator.

In this particular example, the analyst has no control whatsoever, so it stands to reason that they could model the administrator's choice and the ensuing consequences with a single probability distribution $\prob{Q}$. A probability set ``comparing'' different choics of the administrator can then be derived from this distribution: $\prob{P}_C:=\{\prob{Q}^{\RV{YD}|\RV{C}}(\cdot|\alpha)|\alpha\in C\}$. 

In particular, suppose the analyst knows that the administrator will behave in one particular way. First, they will examine every patient and observe some binary feature $\RV{U}_i$ of each  before selecting $\RV{C}$. Furthermore, the analyst is certain the consequences and the the administrator's choice will be as follows:

$\prob{Q}^{\RV{Y}_i|\RV{U}_i\RV{D}_i}(1|\cdot,\cdot)$:
\begin{center}
\begin{tabular}{ c | c | c }
  & $\RV{D}_i=0$ & $\RV{D}_i=1$ \\\hline 
 $\RV{U}_i=0$ & 0 & 0 \\ \hline 
...
\end{center}
\begin{align}
    \prob{Q}{\RV{C}_i|\RV{U}_i}(A|u) &= \begin{cases}
    \delta_{1-\epsilon}(A)&A\in \sigalg{B}([0,1]),u=1\\
    \delta_{\epsilon}(A)&A\in \sigalg{B}([0,1]),u=0
    \end{cases}
\end{align}
where
\begin{align}
    \prob{Q}^{\RV{D}_i|\RV{C}_i}(A|c) &= \delta_1(A)c + \delta_0(A)(1-c)&A\subset\{0,1\},c\in \{\epsilon,1-\epsilon\}
\end{align}
and
\begin{align}
    \prob{Q}^{\RV{U}_i}(\{u\}) &= 0.5& u\in \{0,1\}
\end{align}
this implies
\begin{align}
    \prob{Q}^{\RV{Y}_i|\RV{C}_i}(A|c) &= \begin{cases}
    \delta_1(A) & c=1-\epsilon,A\subset \{0,1\}\\
    \delta_0(A) & c=\epsilon,A\subset \{0,1\}
    \end{cases}
\end{align}


In this case, index permutability will fail.
\end{example}
