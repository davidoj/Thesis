...
To show $\RV{Y}\CI\RV{X}|\RV{Z}\utimes\RV{D}$, consider arbitrary $\RV{V}:E\to V$. Due to the exchangeability of $\kernel{T}$, for any $A\in \sigma(\sigalg{H}^n\utimes\RV{X}_I)$, $I\subset\mathbb{N}$ any size $n$ swap function $\rho^*$ and any $A\in\sigalg{H}^n$:

\begin{align}
    \int_A \mathbb{E}[\RV{V}|\sigma(\RV{X}_I)] \circ \rho^* d\kernel{T}_d &= \int_{\rho^*(A)} \mathbb{E}[\RV{V}|\sigma(\RV{X}_I)] d(\kernel{T}\rho)_d\\
                                                                                       &= \int_A \mathbb{E}[\RV{V}|\sigma(\RV{X}_I)] d\kernel{T}_d
\end{align}

Consider also, for $i,j\in \mathbb{N}$, swap function $\rho^*_{ji}$, $B\in \sigalg{B}([0,1])$:

\begin{align}
    \int_{\RV{X}_i^{-1}(B)} \mathbb{E}[\RV{V}|\sigma(\RV{X}_j)] \circ \rho^*_{ji} d\kernel{T}_d &= \int_{\rho^*_{ji}(\RV{X}_i^{-1}(B))} \mathbb{E}[\RV{V}|\sigma(\RV{X}_j)] d(\kernel{T}\rho_{ij})_d\\
                                                                               &= \int_{\RV{X}^{-1}_{j}(B)} \mathbb{E}[\RV{V}|\sigma(\RV{X}_j)] d\kernel{T}_d\\
                                                                               &= \int_{\RV{X}_j^{-1}(B)} \RV{V} d\kernel{T}_d\\
                                                                               &= \int_{\rho_{ji}^*(\RV{X}_j^{-1}(B))}\RV{V}\circ \rho^*_{ji} d(\kernel{T}\rho_{ji})_d\\
                                                                               &= \int_{\RV{X}_i^{-1}(B)} \RV{V}d\kernel{T}_d
\end{align}

Thus $\mathbb{E}[\RV{V}|\sigma(\RV{X}_j)]\circ\rho^*_{ji}$ is a version of $\mathbb{E}[\RV{V}|\sigma({\RV{X}_i})]$.

Define $\RV{W}^n_q := \frac{1}{n}\sum_{i\in [n]} \mathbb{E}[\mathds{1}_{[0,q]}\RV{V}|\sigma(\RV{X}_i)]$. Note that for any size $n$ swap function $\rho^*$, $\RV{W}^n_q\circ\rho^*=\RV{W}_q^n$, therefore $\RV{W}_q^n$ is $\sigalg{H}^n$-measurable. 

Consider $\omega,\omega'$ such that $\RV{W}_q^n(\omega)\neq \RV{W}_q^n(\omega')$. Then there exists no size $n$ swap function $\rho^*$ such that $\RV{X}_{[n]}(\omega)=\RV{X}_{[n]}(\rho^*(\omega'))$. Without loss of generality, suppose $\RV{X}_1(\omega)\leq\RV{X}_2(\omega)\leq...\leq\RV{X}_n(\omega)$ and $\RV{X}_1(\omega')\leq\RV{X}_2(\omega')\leq...\leq\RV{X}_n(\omega')$. Then there is some first index $j$ for which $\RV{X}_{j}(\omega)> \RV{X}_{j}(\omega')$. and some and some rational $q$ such that $\RV{X}_j(\omega)>q>\RV{X}_j(\omega')$. Then $\sum_{i}^n \mathds{1}_{[0,q)}(\RV{X}_i(\omega)) > \sum_i^n \mathds{1}_{[0,q)}(\RV{X}_i(\omega'))$ and so $\RV{Z}^n(\omega)\neq\RV{Z}^n(\omega')$ also.

Thus $\RV{W}_q^n$ is in fact $\sigma(\RV{Z}^n)$ measurable. Define $\sigalg{I}^n:=\vee_{n\to\infty} \sigma(\RV{Z}^n)$. Then $\sigalg{I}^1\supset\sigalg{I}^2\supset...\supset\sigma(\RV{Z})=\cap_{i}^\infty \sigalg{I}^i$. In addition, but the $\sigalg{H}^n$-measurability of $\RV{Z}^{>n}$, $\sigalg{I}^n\subset\sigalg{H}^n$ and $\sigalg{I}\subset\sigalg{H}$.

For $A\in\sigalg{I}^n$, $d\in D$:

\begin{align}
    \int_A \RV{W}_q^n d\kernel{T}_d &= \frac{1}{n}\sum_{i\in[n]} \int_A \mathbb{E}[\mathds{1}_{[0,q)}\circ\RV{V}|\sigma(\RV{X}_i)] d\kernel{T}_d\\
                                  &= \frac{1}{n}\sum_{i\in[n]} \int_{\rho_{ij}^*(A)} \mathbb{E}[\mathds{1}_{[0,q)}\circ|\sigma(\RV{X}_i)]\circ\rho_{ij}^*d(\kernel{T}\rho_{ij})_d\\
                                  &= \int_{A} \mathbb{E}[\mathds{1}_{[0,q)}\circ \RV{V} \sigma(\RV{X}_j)]d\kernel{T}_d\\
                                  &= \int_{A} \mathds{1}_{[0,q)}\circ \RV{V} d\kernel{T}_d
\end{align}

Where the last line follows from the fact that $\sigalg{I}^n\subset \sigma(\RV{X}_j)$. Therefore $\RV{W}^n_q$ is a version of $\mathbb{E}[\mathds{1}_{[0,q)}\circ|\sigalg{I}^n]$. Furthermore $\RV{W}^1_q,...,\RV{W}^n_q,..$ is a backwards martingale with respect to $\sigalg{I}^1,....,sigalg{I}^n,..\sigalg{I}$ and so it goes to a limit $\RV{W}_q=\mathbb{E}[\mathbb{E}[\mathds{1}_{[0,q)}\circ \RV{V} |\sigma(\RV{Z})]$.

Define $\RV{W}^{n}_{q,r} = \frac{1}{n}\sum_{i\in[n]} \mathbb{E}[\mathds{1}_{[0,q)}\circ\RV{V}|\mathds{1}_{[0,r)}\circ\RV{X}_i] \mathds{1}_{[0,r)}\circ\RV{X}_i$.

For $A\in \sigalg{I}^n$, any $d\in D$ and any $j\in [n]$:

\begin{align}
    \int_A \RV{W}^{n}_{q,r} (\omega)d\kernel{T}_d(\omega) &= \int_A \frac{1}{n}\sum_{i\in [n]} \mathbb{E}[\mathds{1}_{[0,q)}\circ\RV{V}|\mathds{1}_{[0,r)}\circ\RV{X}_i](\omega) \mathds{1}_{[0,r)}\circ\RV{X}_i(\omega) d\kernel{T}_d(\omega)\\
    &= \int_A\mathbb{E}[\mathds{1}_{[0,q)}\circ\RV{V}|\mathds{1}_{[0,r)}\circ\RV{X}_j](\omega) \mathds{1}_{[0,r)}\circ\RV{X}_j(\omega)  d\kernel{T}_d(\omega)\\
    &= \int_{A\cap \RV{X}_j^{-1}([0,r))}\mathds{1}_{[0,q)}\circ \RV{V} (\omega)d\kernel{T}_d(\omega)\\
    &= \int_A \mathds{1}_{[0,q)}\circ\RV{V}(\omega) \mathds{1}_{[0,r)}\circ\RV{X}_i(\omega) d\kernel{T}_d(\omega)
\end{align}

The last line follows from the fact that $A\cap \RV{X}_j^{-1}([0,r))\in [0,r)\}\in \sigma(\RV{X}_j)$. Thus $\RV{W}^{n}_{q,r}$ is a version of $\mathbb{E}[\mathds{1}_{[0,q)}\circ\RV{V} \mathds{1}_{[0,r)}\circ \RV{X}_j|\sigalg{I}^n]$. As before, we have a backward martingale that goes to $\mathbb{E}[\mathds{1}_{[0,q)}\circ\RV{V} \mathds{1}_{[0,r)}\circ \RV{X}_j|\sigalg{I}^n]$.

So

\begin{align}
    \mathbb{E}[\mathds{1}_{[0,q)}\circ\RV{V} \mathds{1}_{[0,r)}\circ \RV{X}_j|\sigalg{I}^n] &= \lim_{n\to\infty}\frac{1}{n}\sum_{i\in [n]} \mathbb{E}[\mathds{1}_{[0,q)}\circ\RV{V}|\mathds{1}_{[0,r)}\circ\RV{X}_i] \mathds{1}_{[0,r)}\circ\RV{X}_i\\
    &= \int_A \left(\mathbb{E}[\mathds{1}_{[0,q)}\circ\RV{V}|\sigma(\RV{X}_i)]\right) d\kernel{T}_d\right)\\
    && \left(\frac{1}{n}\sum_{i\in[n]} \int_A \mathbb{E}[\mathds{1}_{[0,r)}\circ\RV{X}|\sigma(\RV{X}_i)] d\kernel{T}_d\right)
\end{align}
