...
For standard measurable $(E,\sigalg{E})$ and $n\in \mathbb{N}$, a size $n$ swap function $\rho^{*n}:E\to E$ is a swap function associated with a permutation $\rho^{\prime n}$ with the property $i>n\implies \rho^{\prime n}(i)=i$. Given random variables $\RV{X}_i:E\to X_1$ for each $i\in\mathbb{N}$, the size $n$ \emph{exchangeable $\sigma$-algebra} with respect to $\RV{X}:=\utimes_{i\in\mathbb{N}}\RV{X}_i$, denoted $\sigalg{H}^n\subset\sigma(\RV{X})$, is the $\sigma$-algebra $\{A|A\in\sigma(\RV{X}),\rho^{*n}(A)=A\forall \text{ size $n$ swap functions }\rho^{*n}:E\to E\}$.

Define the exchangeable $\sigma$-algebra $\sigalg{H}:=\int_{n\in\mathbb{N}} \sigalg{H}^n$
\end{definition}

\begin{lemma}
The size $n$ exchangeable sigma algebras $\sigalg{H}^n$ on $(E,\sigalg{E})$ with respect to $\RV{X}:=\utimes_{i\in\mathbb{N}}\RV{X}_i$ are $\sigma$-algebras for all $n\in \mathbb{N}$.
\end{lemma}

\begin{proof}
Take $A\in\sigalg{H}^n$. By assumption, for any $\omega\in A$, $\rho^{*}(\omega)\in A$ for all size $n$ swap functions $\rho^{*}$. Consider $\omega\not\in A$, and suppose there is some swap function $\rho^{*}$ such that $\rho^{*}(\omega)\in A$. By definition, the permutation $\rho^{\prime}$ has an inverse $\rho^{\prime -1}$ which is also a size $n$ permutation. By construction, $(\rho^{-1})^*$ is also the inverse of $\rho^*$. Thus $(\rho^{-1})^*(\omega)\not\in A$, a contradiction. Thus $E\setminus A\in \sigalg{H}$.

For any invertible function $f:E\to E$, $f(E)=E$. Thus $E\in \sigalg{H}$.
...
$\sigalg{H}$ is an intersection of $\sigma$-algebras, so it is also a $\sigma$-algebra.
\end{proof}

\begin{lemma}\label{lem:partialfreq_exchangeable}
Given $\RV{Z}^{m,n}$, the size $n$, $m$-tuple relative frequency of $\RV{X}:=\utimes_{i\in \mathbb{N}} \RV{X}_i$, define $\sigalg{I}^{m,n}_A:=\vee_{i\in[n,\infty)}\sigma(\RV{Z}^{m,i})$. Then $\sigalg{I}^{m,n}\subset\sigalg{H}^n$ for any $m$, and also $\sigalg{I}^{m,\infty}:=\cap_{n\in\mathbb{N}} \sigalg{I}^{m,n}\subset\sigalg{H}$.
\end{lemma}

\begin{proof}
For any $A\in \sigalg{G}$, size $n$ swap function $\rho^{*}: E\to E$ associated with some finite permuation of the natural numbers $\rho^{\prime}:\mathbb{N}\to \mathbb{N}$, we have $\RV{Z}^{m,n}_A\circ \rho^{*} = \RV{Z}^{m,n}_A$. Thus $(\RV{Z}^{m,n}_A)^{-1}(B) = (\RV{Z}^{m,n}_A\circ (\rho^{*})^{-1})^{-1}(B) = \rho^{*}((\RV{Z}^{m,n}_A)^{-1}(B))$ for $B\in \sigalg{B}([0,1])$. Because $\rho^{*}$ is injective, for any collection of sets $\{A_i|i\in R\subset\mathbb{N}\}$ for which $\rho^{*}(A_i)=A_i$, we have $\rho^{*}(\cap_{i\in R} A_i)=\cap_{i\in R} \rho^*(A_i) = \cap_{i\in R} A_i$.

We have $\sigalg{I}^{m,n}=\vee_{i\in[n,\infty)}\vee_{A\in \sigalg{G}} \sigma(\RV{Z}^{m,i}_A)$.

A generating $\pi$-system for $\sigalg{I}^{m,n}$ is $\sigalg{J}^{m,n}:=\{\cap_{i=1}^r B_i|B_i\in \sigma(\RV{Z}^{m,j}_{A_i}),A_i\in\sigalg{G},j\in[n,\infty),r\in\mathbb{N})$ where $A_i$ is an enumeration of the sets in $\sigalg{G}$. In the previous paragraph we showed that symmetry with respect to $\rho^*$ is preserved for intersections, thus $\rho^*(A) = A$ for all $A\in \sigalg{J}^{m,n}$ for all size $n$ swap functions $\rho^*$.

Thus $\sigalg{J}^{m,n}\subset\sigalg{H}^n$ and, because $\sigalg{J}^{m,n}$ is a $\pi$-system, $\sigalg{I}^{m,n}\subset\sigalg{H}^n$ by the monotone class theorem.

As $\sigalg{I}^{m,\infty}=\cap_{n\in\mathbb{N}} \sigalg{I}^{m,n}$ we have $\sigalg{I}^{m,\infty}\subset\sigalg{H}$ also.
\end{proof}
...
Define $\sigalg{I}^n_q = \vee_{N}^\infty \sigalg{Z}^n_q$. Then $\sigalg{I}^1_q\supset\sigalg{I}^2_q\supset...\sigalg{I}_q:=\cap_{i\in\mathbb{N}}\sigalg{I}^i_q = \sigalg{Z}^\infty_q$.

Let $\sigalg{I}^n=\vee_{q\in\mathbb{Q}} \sigalg{I}^n_q$. Then $\sigalg{I}^1\supset \sigalg{I}2\supset...\supset\sigalg{I}$.

By Lemma \ref{lem:partialfreq_exchangeable} for $m=1$, $\sigalg{I}^n\subset\sigalg{H}^n$.
...
Let $\RV{Z}^{m,n}_q$ be a partial frequency as in Definition \ref{def:partial_freq} where $q$ stands for the set $[0,q]$.

Let $\sigalg{I}^{m,n}=\vee_{q\in\mathbb{Q}} \sigalg{I}^{m,n}_q$. Then $\sigalg{I}^{m,1}\supset \sigalg{I}^{m,2}\supset...\supset\sigalg{I}^{m,\infty}$.

By Lemma \ref{lem:partialfreq_exchangeable}, $\sigalg{I}^{m,\infty}\subset\sigalg{H}$.
...
To show $\RV{Y}\CI\RV{X}|\RV{Z}\utimes\RV{D}$, we will make a graphical argument. For $B,C\subset\mathbb{N}$:

\begin{align}
    \kernel{T}^{\RV{Z}\RV{X}_B\RV{X}_C\RV{Y}} &= \begin{tikzpicture}
